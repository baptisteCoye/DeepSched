...
# def post_process_probabilist(schedulable, raw_solution):
#     processed_solution = numpy.zeros(len(raw_solution))
#     mask = schedulable*raw_solution
#     index = get_max_index()
#     for i in range(3):
#         index_and_value = get_max_index(raw_solution)
#         index = index_and_value[0]
...
def get_max_index(vector):
    tmp = vector[0]
    index = 0
    index_and_value = list()
    index_and_value.append((index,tmp))
    for i in range(len(vector)):
        if vector[i] > index_and_value[0][1]:
            index_and_value = list()
            index_and_value.append((i,vector[i]))
        elif vector[i] == index_and_value[0][1]:
            index_and_value.append((i, vector[i]))
    random.shuffle(index_and_value)
...
def probabilist_solution_real(schedulable, dag_solution):
    mask = numpy.zeros(len(dag_solution))
    for i in range(len(schedulable)):
        mask[i] = dag_solution[i][0]*schedulable[i]
    ...
    if len(index) > 2:
        ...
        mask[index[0]] = 0
        if not empty(mask):
            third_index = get_max_index_list(mask)
            if (len(third_index) > 1):
                for i in range(len(third_index)):
                    prob_sol[third_index[i]] = 2 / len(third_index)

            else:
                prob_sol[third_index[0]] = 1
                mask[third_index[0]] = 0
                if not empty(mask):
                    last_index = get_max_index_list(mask)
                    for i in range(len(last_index)):
                        prob_sol[last_index[i]] = 1 / len(last_index)
            return prob_sol
        else:
            return prob_sol
